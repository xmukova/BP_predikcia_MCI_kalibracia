# DEFINICIA PIPELINE PRE PREDSPRACOVANIE s pouzitim oversampling - smote
# rovnaky princip ako pipeline.py, ale s doplnenim oversamplingu
...
class CustomFeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        l1 = LogisticRegression(penalty="l1", solver="saga", max_iter=1000, random_state=42 )
        l1.fit(X, y)
...
def get_feature_selector(feature_selection=None, k_features=len(FEATURES)):    
    ...
    # Custom - kombinacia MI, RFE, Random Forest a L1 regularization
...
def get_model(model_name, model_parameters=None):
    ...
    if model_name == "lr":
        return LogisticRegression(
            max_iter=1000,
            random_state=42,
            #class_weight="balanced",       # robime oversampling, nie vahovanie tried
            **model_parameters
        )
...
def create_pipeline_complete(model_name, feature_selection=None, k_features=40, model_parameters=None, ):
    pipeline = Pipeline([
        ("preprocessing", create_pipeline_first_part()),
        # ("sampler", SMOTENC(categorical_features=categorical_indices, random_state=42)),
        ("feature_selection", get_feature_selector(feature_selection, k_features)),
        ("sampler", SMOTE(random_state=42)),        # pridanie SMOTE do pipeline, ktory vytvori synteticke vzorky pre mensinovu triedu na zaklade existujucich vzoriek a ich najblizsich susedov
